...
# TODO CHECK FOR FILE-EXISTS AND -AGE TO DOWNLOAD ONLY IF MAXIMUM AGE IS REACHED
...
with open(DATA_FILE) as csv_file:
    ...
    for row in CSV_READER:
        if LINE_COUNT == 0:
            L_TABLE.append(['Name', 'Fälle/100k', 'RwK', '50inW'])
            LINE_COUNT += 1
        else:
            if float(row[16]) > 0 and float(row[23]) > 1:
                ...
                L_TABLE.append([row[1], row[16], row[23], weeks_left])
                LINE_COUNT += 1
